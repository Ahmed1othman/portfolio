...
class projectsRequest extends FormRequest
{
    ...
    public function rules()
    {
         switch ($this->method()) {
            ...
            case 'POST': {
                    return [
                        'title' => 'required|string|min:2|max:150|regex:/^[A-Za-z0-9-pL\s\-]+$/u|unique:projects,title->en'. $this->id,
                        'title_ar' => 'required|string|min:2|max:150|regex:/^[أ-ي-pL\s\-]+$/u|unique:projects,title->ar'. $this->id,
                        'notes' => 'required|string|min:2|regex:/^[A-Za-z0-9-pL\s\-]+$/u|unique:projects,notes->en' . $this->id,
                        'notes_ar' => 'required|string|min:2|regex:/^[0-9-أ-ي-pL\s\-]+$/u|unique:projects,notes->ar' . $this->id,
