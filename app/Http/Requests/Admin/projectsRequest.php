...
class projectsRequest extends FormRequest
{
    ...
    public function rules()
    {
         switch ($this->method()) {
            ...
            case 'POST': {
                    return [
                        'title' => 'required|string|min:2|max:150|unique:projects,title->en'. $this->id,
                        'title_ar' => 'required|string|min:2|max:150|unique:projects,title->ar'. $this->id,
                        'notes' => 'required|string|min:2|unique:projects,notes->en' . $this->id,
                        'notes_ar' => 'required|string|min:2|unique:projects,notes->ar' . $this->id,
